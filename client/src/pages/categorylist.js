...
const Categorylist = () => {
  ...
  async function GetCatProduct() {
    try {
      let url;
      if (auth.user) {
        url = `https://ayushreactbackend.onrender.com/api/v1/product/product-CategoryWise/${Params.id}/${auth.user._id}`;
      } else {
        url = `https://ayushreactbackend.onrender.com/api/v1/product/product-CategoryWise/${Params.id}/65f9bb4749049ec84f1de5be`;
      }
